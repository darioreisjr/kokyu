...
describe('getWeekStart / getWeekDays', () => {
  ...
  it('starts the week on Sunday when weekStartsOn is 0', () => {
    const start = getWeekStart(new Date(2026, 7, 29), 0);
    expect(toDateKey(start)).toBe('2026-08-23');
  });
...
describe('formatWeekdayShort', () => {
  it('formats a capitalized full weekday name', () => {
    expect(formatWeekdayShort(new Date(2026, 7, 29))).toBe('Sábado');
  });
});
