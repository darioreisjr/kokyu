...
/** The one confirmation dialog every destructive Tempo Livre action shares — fed by `useConfirmAction`, never built ad hoc per call site. */
export function ConfirmActionDialog({ request, onConfirm, onCancel }: ConfirmActionDialogProps) {
  return (
    <Dialog
      ...
      maxWidth="xs"
      fullWidth
      aria-labelledby="leisure-confirm-action-title"
    >
      <DialogTitle id="leisure-confirm-action-title">{request?.title}</DialogTitle>
      <DialogContent>
