...
/** The one confirmation dialog every destructive Metas action shares — fed by `useConfirmAction`, never built ad hoc per call site. */
export function ConfirmActionDialog({ request, onConfirm, onCancel }: ConfirmActionDialogProps) {
  return (
    <Dialog
      ...
      maxWidth="xs"
      fullWidth
      aria-labelledby="goals-confirm-action-title"
    >
      <DialogTitle id="goals-confirm-action-title">{request?.title}</DialogTitle>
      <DialogContent>
