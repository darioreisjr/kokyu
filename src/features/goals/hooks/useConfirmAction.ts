...
export interface UseConfirmActionResult {
  ...
  /** Every destructive Metas action calls through here instead of its own dialog — respects Settings → "Confirmar ações importantes" once, centrally. */
...
/**
 * When "Confirmar ações importantes" is off, `request()` runs `onConfirm` immediately — no
 * dialog, no double confirmation on top of a preference that already says "skip this." Never a
 * second, feature-local copy of that preference (same pattern as `leisure`/`nutrition`).
 */
