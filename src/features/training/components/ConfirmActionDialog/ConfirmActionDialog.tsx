...
/** The one confirmation dialog every destructive Treinamento action shares — fed by `useConfirmAction`, never built ad hoc per call site. */
export function ConfirmActionDialog({ request, onConfirm, onCancel }: ConfirmActionDialogProps) {
  return (
    <Dialog
      ...
      maxWidth="xs"
      fullWidth
      aria-labelledby="training-confirm-action-title"
    >
      <DialogTitle id="training-confirm-action-title">{request?.title}</DialogTitle>
      <DialogContent>
